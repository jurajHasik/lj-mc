...
! Initialize RanLux PRNG
call RLuxGo(lux_lvl, seed, 0, 0)
write(*,'("Seed",16X,I10)') seed
write(*,'("-sample output-")')
call RanLux(rnd10,size(rnd10))
do i=1, 10
    write(*,*) rnd10(i)
enddo
...
do i=1, eqS
    ...
    call RanLux(rndN,size(rndN))
    do k=1, N
        call RanLux(rnd4,size(rnd4))
