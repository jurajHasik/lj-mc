...
integer :: N, A
real*8 :: temp, beta, rT, rB, rD, rDens, pres, rP
...
do i=1, eqS
    ...
    if(mod(i,sclMvRt) .eq. 0) then
        ...
        call sclMv(N, rP, sclF, rnd2, r, e6, e12, vir6, vir12, eLRC6, &
    ...
    if(mod(i,sclMvAdjstRt) .eq. 0) then
        ...
        accSclMv = 0
        write(*,'("EQ STEP: ",I10," sclF: ",1f20.10)') i, sclF
    endif
enddo
...
subroutine sclMv(N, ext_p, sclF, rnd2, r, e6, e12, v6, v12, eLRC6, &
    &eLRC12, vLRC6, vLRC12, rB, rD, rL, rV, rCut, rCut6, rDens, acc)
    implicit none

    integer, intent(in) :: N
    real*8, dimension(2), intent(in) :: rnd2
    real*8, dimension(3,N), intent(inout) :: r
    real*8, intent(in)    :: sclF, ext_p, rB
